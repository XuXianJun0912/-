...
export default {
  ...
  methods: {
    ...
    getData() {
      this.$axios
        .get(
          `/backend/customer/list?pageNum=${this.page.pageNum}&pageSize=${this.page.pageSize}&searchValue=${this.imform.searchValue}`
        )
        .then((resp) => {
      ...
      if (this.multipleSelection.length >= 1) {
        this.$confirm("是否启用", "提示", {
          ...
          .then(() => {
            ...
            this.ostatus = "enable";
            this.$axios
              .put("backend/customer/" + this.ostatus + "/" + params.userIds)
              .then((resp) => {
                if (200 == resp.data.code) {
                  console.log(resp);
                  this.words();
      ...
      if (this.multipleSelection.length >= 1) {
        this.$confirm("是否删除", "提示", {
          ...
          .then(() => {
            ...
            params.userIds = params.userIds.join(",");
            this.$axios
              .delete("backend/customer/delete/" + params.userIds)
              .then((resp) => {
                if (200 == resp.data.code) {
                  console.log(resp);
                  this.words();
