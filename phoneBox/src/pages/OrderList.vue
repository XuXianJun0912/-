    ...
    <van-pull-refresh v-model="refreshing" @refresh="onRefresh">
      ...
      >
        <div
          class="allList"
          id="allList"
          v-for="(item, index) in listData.rows"
          :key="index"
          @click="Detail(item)"
        >
          <ul class="list">
...
export default {
  ...
  methods: {
    getData() {
      this.$axios
        .get(this.$store.state.url + "/app/order/list", {
          params: {
        ...
        .then((resp) => {
          ...
          if (200 == resp.data.code) {
            this.pageSize = resp.data.total;
            this.listData.total = resp.data.total;
            this.listData.rows = resp.data.rows;
          }
        });
    ...
    //   //   arr.data = this.list[0].rows;
    //   //   arr.total = this.list[0].total;
      ...
      setTimeout(() => {
        ...
        for (var i = 0; i < this.temporary.length; i++) {
          if (this.temporary[i].status == 1) {
            ...
            this.temporary[i].status = "已下单提取单";
